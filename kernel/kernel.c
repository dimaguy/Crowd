...
typedef struct syscall_desc_t {
    uint64_t syscall_num;
    uint64_t syscall_addr;
} __attribute__((packed)) syscall_desc_t;



//generate system call descriptor table
syscall_desc_t syscall_desc[256];
syscall_desc_ptr_t syscall_desc_ptr = {
  .limit = sizeof(syscall_desc) - 1,
  .base = (uint64_t)&syscall_desc
};
*/
...
//To satisfy printf.c
//TODO: create a buffer for the terminal. Maybe 64 chars?
...
void _start(struct stivale2_struct *stivale2_struct) {
    ...
    load_idt();
    printf("[DONE]\n");
    printf("Sending on the serial port\n");
    serial_print("Hello, world!\0");
    printf("check the serial port\n");
    // Start Memory Manager
    printf("Starting Memory Manager...");
    init_mm(stivale2_struct);
    printf("[DONE]\n");
    //pmtest();
